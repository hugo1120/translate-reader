...
from src.config.settings import _DEFAULT_TRANSLATION_PROMPTS, resolve_translation_config, resolve_translation_prompt_config
...
class OpenAICompatibleTranslator:
    def translate_texts(self, texts, model, base_url, api_key=None, context_snapshot=None):
        ...
        prompt_config = resolve_translation_prompt_config()
    ...
    def translate_texts_with_metadata(self, texts, model, base_url, api_key=None, context_snapshot=None):
        ...
        if not non_empty_items:
            empty_rounds = [
                {"name": "draft", "translatedTexts": list(final_texts), "usage": _empty_usage()},
                {"name": "contextual", "translatedTexts": list(final_texts), "usage": _empty_usage()},
                {"name": "final", "translatedTexts": list(final_texts), "usage": _empty_usage()},
            ]
            return {
