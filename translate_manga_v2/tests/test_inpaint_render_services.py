...
from translate_manga.core.color.service import extract_bubble_colors
...
def test_inpaint_page_delegates_to_saber(monkeypatch):
    ...
    monkeypatch.setattr("translate_manga.core.inpaint.service.run_saber_task", fake_run_saber_task)
...
def test_render_page_delegates_to_saber(monkeypatch):
    ...
    monkeypatch.setattr("translate_manga.core.render.service.run_saber_task", fake_run_saber_task)
...
def test_extract_bubble_colors_delegates_to_saber(monkeypatch):
    ...
    monkeypatch.setattr("translate_manga.core.color.service.has_saber_48px_color_model", lambda: True)
    monkeypatch.setattr("translate_manga.core.color.service.run_saber_task", fake_run_saber_task)
...
def test_extract_bubble_colors_enriches_results_with_background_metrics(tmp_path, monkeypatch):
    ...
    image = Image.new("RGB", (80, 80), color=(245, 245, 245))
    draw = ImageDraw.Draw(image)
    draw.line((10, 10, 70, 70), fill=(0, 0, 0), width=3)
    image.save(image_path)

    monkeypatch.setattr("translate_manga.core.color.service.has_saber_48px_color_model", lambda: True)
    monkeypatch.setattr(
        "translate_manga.core.color.service.run_saber_task",
        lambda *args, **kwargs: {
            "colors": [
                {
                    "textColor": "#111111",
    ...
    result = extract_bubble_colors(
        ...
        bubble_coords=[[8, 8, 72, 72]],
        textlines_per_bubble=[[{"polygon": [[20, 20], [60, 20], [60, 30], [20, 30]], "direction": "h"}]],
    )
...
def test_extract_bubble_colors_returns_empty_colors_when_48px_model_missing(monkeypatch):
    monkeypatch.setattr("translate_manga.core.color.service.has_saber_48px_color_model", lambda: False)
