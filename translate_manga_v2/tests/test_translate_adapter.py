...
def test_translate_texts_with_metadata_balanced_quality_keeps_two_rounds_for_dense_batch(monkeypatch):
    responses = [
        "<|1|>初1\n<|2|>初2\n<|3|>初3\n<|4|>初4\n<|5|>初5",
        "<|1|>终1\n<|2|>终2\n<|3|>终3\n<|4|>终4\n<|5|>终5",
        "<|1|>初6\n<|2|>初7",
    ...
    assert len(calls) == 4
    assert [round_payload["name"] for round_payload in result["rounds"]] == ["draft", "final"]
    assert result["translatedTexts"] == ["终1", "终2", "终3", "终4", "终5", "终6", "终7"]
...
def test_translate_texts_with_metadata_uses_configured_prompts(monkeypatch):
    ...
    monkeypatch.setattr("openai.OpenAI", FakeOpenAI)
    monkeypatch.setattr(
        "translate_manga.core.translate.openai_compatible.resolve_translation_prompt_config",
...
def test_translate_texts_with_metadata_selects_prompt_profile_from_context(monkeypatch):
    captured = {"prompt_profiles": []}
    responses = [
        ("<|1|>draft", type("Usage", (), {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2})()),
        ("<|1|>context", type("Usage", (), {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2})()),
        ("<|1|>final", type("Usage", (), {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2})()),
    ]
    ...
    def fake_resolve_translation_prompt_config(project_root=None, settings=None, prompt_profile=None):
        captured["prompt_profiles"].append(prompt_profile)
        return {
            "system": f"{prompt_profile} system",
            "rounds": {
                "draft": f"{prompt_profile} draft",
                "contextual": f"{prompt_profile} contextual",
                "final": f"{prompt_profile} final",
            },
        }
    ...
    monkeypatch.setattr(
        "translate_manga.core.translate.openai_compatible.resolve_translation_prompt_config",
        fake_resolve_translation_prompt_config,
        raising=False,
    )
    ...
    result = OpenAICompatibleTranslator().translate_texts_with_metadata(
        ...
        model="mimo-v2.5-pro",
        base_url=TEST_BASE_URL,
        context_snapshot={"promptPreset": "english"},
    )

    assert result["translatedTexts"] == ["final"]
    assert captured["prompt_profiles"] == ["english"]
