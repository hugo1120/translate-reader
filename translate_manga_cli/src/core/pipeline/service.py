...
from src.config.settings import load_settings, resolve_path_value, resolve_translation_config
...
def _resolve_bubble_readability_style(style, color, coords):
    ...
    bg_rgb = color.get("autoBgColor") or _parse_hex_color(fill_color)
    fg_rgb = color.get("autoFgColor") or _parse_hex_color(text_color)
    ...
    if bg_rgb is None and fg_rgb is None:
        return {
            ...
            "strokeWidth": int(style["strokeWidth"]),
        }

    background_is_dark = False
    bg_luminance = _relative_luminance(bg_rgb)
    if bg_luminance is not None:
        background_is_dark = bg_luminance < 128
    else:
        fg_luminance = _relative_luminance(fg_rgb)
        background_is_dark = fg_luminance is not None and fg_luminance >= 160
    ...
    is_tiny_box = min(box_width, box_height) <= 28 or (box_width * box_height) <= 900

    stroke_width = 0
    if background_is_dark and style["strokeEnabled"] and int(style["strokeWidth"]) > 0 and not is_tiny_box:
        stroke_width = 1

    return {
        "textColor": "#FFFFFF" if background_is_dark else "#111111",
        "fillColor": fill_color,
        "strokeEnabled": style["strokeEnabled"],
        "strokeColor": "#000000" if background_is_dark else "#FFFFFF",
...
def preprocess_page(source_path, saber_session=None):
...
def retry_preprocess_page_for_ocr(source_path, saber_session=None):
    ...
    try:
        ...
        retry_payload = preprocess_page(str(temp_output), saber_session=saber_session)
...
def run_page_pipeline(
    ...
    if preprocessed_payload is not None:
        ...
        ocr = _call_ocr_page(source_path, detection["bubbleCoords"], detection["textlinesPerBubble"])
    ...
    if translated_texts is None:
        ...
        if retry_state.get("shouldRetry"):
            ...
            retry_applied = False
            try:
                retry_payload = retry_preprocess_page_for_ocr(source_path, saber_session=saber_session)
